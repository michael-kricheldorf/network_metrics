...
if (cur.fetchone()[0] == 0):
        cur = con.execute("CREATE TABLE {0}(DATETIME TEXT, FROM_ADDR TEXT, TO_ADDR TEXT, RSSI TEXT, SNR TEXT);".format(table_name))
    ...
    try:
        while True:
            ...
            for i in range(len(data)):
                # read in SNR value (0xc0, 0x23, SNR, 0xc0)
                # then multiply by 0.25 to get the actual snr
                
                # read in RSSI values (0xc0, 0x23, RSSI, 0xc0)
                # if the frequency is less than 820E6, then subtract 7 from the raw value
                # what is the frequency?

                # gather SNR hex
                if data[i] == 0x24 and data[i-1] == 0xc0:
                    snr = data[i+1]
                # gather RSSI hex
                elif data[i] == 0x23 and data[i-1] == 0xc0:
                    rssi = data[i+1]

                # gather IPv4 from and to addresses
            ...
            return_string = "INSERT INTO {0} VALUES ('{1}', '{2}', '{3}', '{4}', '{5}')".format(
                table_name, t.strftime("%Y-%m-%d %H:%M:%S"), from_addr, to_addr, rssi, snr)
            print(return_string)   
